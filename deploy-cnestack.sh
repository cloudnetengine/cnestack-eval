...
CNESTACK_EVAL_DIR="${TEST_SRC_DIR}/cnestack-eval"

cd ${TEST_SRC_DIR}

cd $OVS_DIR
TARGET_BRANCH="cnestack"
...
git apply ${CNESTACK_EVAL_DIR}/ovs-1d8e0f-cnestack-rc1.patch
cp ${CNESTACK_EVAL_DIR}/libcnestack.a $OVS_DIR/lib/
./boot.sh
./configure --disable-ssl --with-dpdk=static --with-logdir=/var/log/openvswitch --with-rundir=/var/run/openvswitch CFLAGS=" -DCNE_NET -Ofast -msse4.2 -mpopcnt "
